async function init() {
data = await d3.csv('us-states.csv', function(d) {
	return {
		cases: +d.cases,
		deaths: +d.deaths
	};
});
axisLeft = d3.scalelog().domain(0, d3.max(cases)).range(0,300);
axisBottom = d3.scalelog().domain(0, d3.max(deaths)).range(0,300);
